...
export class ReflectionEngine {
  ...
  private readonly reflectionTemplates: string[] = [
   ...
   */
  public generateReflectionPrompts(thoughts: ThoughtData[]): string[] {
   ...
   */
  public generatePromptAwareReflectionPrompts(thoughts: ThoughtData[], promptMetadata: PromptMetadata): string[] {
    const prompts: string[] = [];
    const latestThought = thoughts[thoughts.length - 1];
   ...
   */
  public identifyAssumptions(thought: ThoughtData): string[] {
   ...
   */
  public suggestAlternativePerspectives(thought: ThoughtData): string[] {
   ...
   */
  public suggestCorrectiveActions(thought: ThoughtData, promptMetadata: PromptMetadata): string[] {
    ...
    if (thought.promptAlignment < 5) {
      ...
      const missingGoals = promptMetadata.goals.filter(goal => 
        !thought.thought.toLowerCase().includes(goal.toLowerCase())
      );
   ...
   */
  public adjustReflectionStrategy(thoughts: ThoughtData[], promptMetadata: PromptMetadata): {
    focusAreas: string[];
    recommendedApproach: string;
    reflectionFrequency: 'high' | 'medium' | 'low';
    reflectionDepth: 'deep' | 'balanced' | 'light';
  } {
    // Default strategy
    const strategy = {
      focusAreas: ['consistency', 'completeness', 'quality'],
      recommendedApproach: 'balanced',
      reflectionFrequency: 'medium' as 'high' | 'medium' | 'low',
      reflectionDepth: 'balanced' as 'deep' | 'balanced' | 'light'
    };
  ...
  private generateContextAwarePrompts(currentThought: ThoughtData, allThoughts: ThoughtData[]): string[] {
    const prompts: string[] = [];
    ...
    const similarThoughts = allThoughts.filter(t => 
      t.thoughtNumber !== currentThought.thoughtNumber &&
  ...
  private generatePhaseSpecificPrompts(phase: string): string[] {
  ...
  private generateClassificationPrompts(classification: string): string[] {
   ...
   */
  private generatePromptSpecificReflectionPrompts(thought: ThoughtData, promptMetadata: PromptMetadata): string[] {
    const prompts: string[] = [];
   ...
   */
  private generatePromptAlignmentPrompts(thought: ThoughtData, promptMetadata: PromptMetadata): string[] {
    const prompts: string[] = [];
   ...
   */
  private generateTaskTypePrompts(thought: ThoughtData, taskType: string): string[] {
   ...
   */
  private generateComplexityBasedPrompts(thought: ThoughtData, complexity: string): string[] {
   ...
   */
  private getPhaseSpecificCorrections(phase: string, promptMetadata: PromptMetadata): string[] {
  ...
  private cleanAssumption(text: string): string {
    return text
      .trim()
  ...
  private getImplicitAssumptions(classification: string): string[] {
    const implicitAssumptions: Record<string, string[]> = {
  ...
  private prioritizePrompts(prompts: string[]): string[] {
    // Remove duplicates
